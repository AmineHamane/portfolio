 ...
 * NOT real trading performance — a stylized historical series (pink) with a
 * dashed model projection (cyan) and a confidence cone. Brand-accented motif.
 */
...
const FORECAST = [0.8, 0.85, 0.83, 0.9, 0.94, 0.97];
const BAND = 0.06;
...
const lower = [...lowerPts].reverse().map((p) => `L ${x(p.i).toFixed(1)} ${y(p.v).toFixed(1)}`).join(" ");
const conePath = `${upper} ${lower} Z`;

export function ForecastChart() {
  const reduce = useReducedMotion();
  const draw = reduce ? { pathLength: 1, opacity: 1 } : { pathLength: [0, 1] as number[], opacity: [0, 1] as number[] };
    ...
    <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Stylized forecasting chart">
      <defs>
        <linearGradient id="hist-fill" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="rgba(223,122,254,0.30)" />
          <stop offset="100%" stopColor="rgba(223,122,254,0)" />
        </linearGradient>
      ...
      <motion.path d={areaPath} fill="url(#hist-fill)" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 0.6 }} />
      <motion.path
        d={histPath}
        fill="none"
      ...
      <motion.path d={conePath} fill="rgba(34,211,238,0.12)" stroke="none" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 1.6 }} />
      ...
      <motion.circle cx={x(allLen - 1)} cy={y(FORECAST[FORECAST.length - 1])} r={5} fill="#22d3ee" initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 2.5, type: "spring", stiffness: 200 }} />
      {!reduce && (
        <motion.circle cx={x(allLen - 1)} cy={y(FORECAST[FORECAST.length - 1])} r={5} fill="none" stroke="#22d3ee" strokeWidth={1.5} animate={{ r: [5, 16], opacity: [0.7, 0] }} transition={{ duration: 2, repeat: Infinity, ease: "easeOut" }} />
      )}

      <line x1={x(foreStart)} x2={x(foreStart)} y1={PAD} y2={H - PAD} stroke="rgba(255,255,255,0.10)" strokeWidth={1} strokeDasharray="3 4" />
    </svg>
  );
}
